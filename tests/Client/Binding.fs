...
let view model dispatch =
    concat [
        p [] [input [
            ...
            bind.input.string model.name.first (fun first -> { model.name with first = first; } |> SetName |> dispatch)
        ]]
        p [] [input [
            attr.placeholder "Last name"
            bind.input.string model.name.last (fun last -> { model.name with last = last; } |> SetName |> dispatch)
        ]]
